...
def init_accums(rl_learner):  # in rmsprop
    accums = []
    params = rl_learner.get_params()
    for param in params:
...
def concatenate_all_ob(trajs, pa):
    ...
    for i in range(len(trajs)):
        time_steps_total += len(trajs[i]['reward'])

    all_ob = np.zeros(
        (time_steps_total, 1, pa.network_input_height, pa.network_input_width),
        dtype=theano.config.floatX)

    time_steps = 0
    for i in range(len(trajs)):
        for j in range(len(trajs[i]['reward'])):
            all_ob[time_steps, 0, :, :] = trajs[i]['ob'][j]
            time_steps += 1
...
def plot_lr_curve(output_file_prefix, max_rew_lr_curve, mean_rew_lr_curve, slow_down_lr_curve,
    ...
    ax.plot(slow_down_lr_curve, linewidth=2, label='RL mean')
    for k in ref_discount_rews:
...
def get_traj_worker(rl_learner, env, pa, result):

    trajs = []

    for i in range(pa.num_seq_per_batch):
        traj = get_traj(rl_learner, env, pa.episode_max_length)
        trajs.append(traj)
...
def launch(pa, rl_resume=None, render=False, repre='image', end='no_new_job'):
    ...
    nw_len_seqs, nw_size_seqs = task_distribution.generate_sequence_work(pa, seed=42)
    ...
    for ex in range(pa.batch_size + 1):  # last worker for updating the parameters
        ...
        rl_learners.append(rl_learner)

    accums = init_accums(rl_learners[pa.batch_size])

    # --------------------------------------
    print("Preparing for reference data...")
    # --------------------------------------

    ref_discount_rews, ref_slow_down = slowdown_to_cdf.launch(pa, rl_resume=None, render=False, plot=False, repre=repre, end=end)
    mean_rew_lr_curve = []
    max_rew_lr_curve = []
    slow_down_lr_curve = []
    ...
    for iteration in range(1, pa.num_epochs):
        ...
        for ex in range(pa.num_ex):
            ...
            p = Process(target=get_traj_worker,
                        args=(rl_learners[ex_counter], envs[ex_idx], pa, manager_result, ))
            ps.append(p)
            ...
            if ex_counter >= pa.batch_size or ex == pa.num_ex - 1:
                ...
                grads = rl_learners[pa.batch_size].get_grad(all_ob, all_action, all_adv)
        ...
        if iteration % pa.output_freq == 0:
            ...
            pickle.dump(rl_learners[pa.batch_size].get_params(), param_file, -1)
            param_file.close()

            pa.unseen = True
            slowdown_to_cdf.launch(pa, pa.output_filename + '_' + str(iteration) + '.pkl',
...
def main():
    ...
    launch(pa, rl_resume, render, repre='image', end='all_done')
