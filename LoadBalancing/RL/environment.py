...
class Env:
    def __init__(self, pa, nw_len_seqs=None, nw_size_seqs=None,
        ...
        self.pa = pa
        self.render = render
        self.repre = repre       # (Representation)
        ...
        if nw_len_seqs is None or nw_size_seqs is None:
            # generate new work
            self.nw_len_seqs, self.nw_size_seqs = self.generate_sequence_work(self.pa.simu_len * self.pa.num_ex)

            self.workload = np.zeros(pa.num_res)
            for i in range(pa.num_res):
                self.workload[i] = np.sum(self.nw_size_seqs[:, i] * self.nw_len_seqs) / float(pa.res_slot) / \
